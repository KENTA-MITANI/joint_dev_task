...
class Zoo
  ...
  def info_entry_fee(user)
    ...
    case user.age
    when 0..5 then
      kind_of_entry_fee = @entry_fee[:infant]
    when 6..22 then
      kind_of_entry_fee = @entry_fee[:children]
    when 13..64 then
      kind_of_entry_fee = @entry_fee[:adult]
    when 65..120 then
      kind_of_entry_fee = @entry_fee[:senior]
    end
    puts "#{user.name}さんの入場料金は #{kind_of_entry_fee} 円です。"
  end
end
